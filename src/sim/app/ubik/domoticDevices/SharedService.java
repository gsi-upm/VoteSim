...
public abstract class  SharedService extends FixedDomoticDevice  {
    ...
    protected static Color[] colors ={Color.BLUE, Color.CYAN, Color.MAGENTA, Color.YELLOW, Color.GRAY};
    ...
    public SharedService(int floor, HomePieceOfFurniture device3DModel, Ubik ubik)  {
    ...
    /**
     * Añadir un usuario, cambiar color servicio y usuario
     * Por defecto, se toma la primera preferencia del primer agente que entra a servicio
     * @param p 
     */
    ...
    public void removeUser(UserInterface p) {
        users.remove(p);     
        if(users.isEmpty()) this.getDevice3DModel().setColor(null); 
        p.setColor(null);
    }
    ...
    /**
     * Cargar posibles configuraciones del servicio 
     * Este método debe ser extendido en las distintas clases que herenden
     * 
     */
    ...
    public static  SharedService getService(Person p, String serviceName) {
        return (SharedService) p.getUbik().getBuilding().getFloor(p.getFloor()).getDeviceHandler().getDeviceByName(serviceName);               
    }
    
    /**
     * Servicio más cercano a posición. Si checkIfItIsInRoom, da Null si no están en la misma habitación.
     * @return 
     */
    public static SharedService closestSharedService(Person p, boolean checkIfItIsInRoom) {
        Int2D pos= new Int2D(p.getPosition().x,p.getPosition().y);
        SharedService ss= (SharedService) p.getUbik().getBuilding().getFloor(p.getFloor()).getDeviceHandler().getNearestDevice(pos, SharedService.class);        
        //si el servicio no está en la habitación de la persona no lo puede usar se devuelve null
        if(checkIfItIsInRoom && !isInRoom(ss,p)) return null;
        return ss;
    ...
    public  static boolean isInRoom(SharedService ss, Person p){
        if(p==null || ss==null ||  PositionTools.getRoom(p)==null ) return false; 
        ...
        /**
     * Servicio donde los usuarios tengan mayor número de configuraciones queridas comunes.
     * En caso de haber un servicio libre se envía ahí.
     * @return 
     */
    public static SharedService serviceWithMoreCommonWantedConfigurations(UserInterface p) {
        ...
        for(SharedService ss: SharedService.getServices(p.getUbik(), 0)){
            if(ss.getUsers().isEmpty()) return ss;
            float aux = p.getNegotiation().getNumberOfCommonWantedConfigurations(ss) / ss.getUsers().size();
            if(counter<aux) {
                r=ss;
                counter=aux;
            }
      ...
      public static  List<SharedService>  getServices(Ubik ubik, int floor) {        
        List<SharedService> list =  (List) ubik.getBuilding().getFloor(floor).getDeviceHandler().getDevicesInstanceOf(SharedService.class);       
        return list;
